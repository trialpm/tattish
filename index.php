...
// Method 1: Meta tag se
if (preg_match('/<meta name="serialized-session-token" content="&quot;(.*?)&quot;"/', $response, $matches)) {
    $x_checkout_one_session_token = $matches[1];
}

// Method 2: JSON data se
...
// Method 4: Header pattern
if (empty($x_checkout_one_session_token)) {
    if (preg_match('/x-checkout-one-session-token[=:]\s*["\']?([^"\'\s]+)/', $response, $matches)) {
        $x_checkout_one_session_token = $matches[1];
    }
}
...
// Debug - response save karo agar token nahi mila
if (empty($x_checkout_one_session_token)) {
